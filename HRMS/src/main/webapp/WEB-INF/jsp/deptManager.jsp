...
<html>
  ...
  <body style="background:url('<%=request.getContextPath() %>/static/images/back3.jpg') no-repeat">
    ...
    <!-- 添加部门信息，默认是隐藏的 -->
    <div id="AddDialog" class="easyui-window" title="添加用户" style="width:380px;height:300px;margin:auto"   
    	...
    	<form id="myform" method="post">
			...
			<table>
				<tr>
					<td>部门名称:</td>
					<td><input type="text" name="dept_id" class="easyui-validatebox" validType="midLength[1,20]" value="" /></td>
				</tr>
				<tr>
					<td>部门名称：</td>
					<td><input type="text" name="dept_name" class="easyui-validatebox" validType="midLength[1,50]" value="" /></td>
				</tr>
				<tr align="center">
					<td colspan="2">
						<br>
						<a id="btn1" class="easyui-linkbutton" onclick="save()">确定</a>
					</td>
				</tr>   					 					    					    					    					    					    					    					    					
			</table>
   		</form>
   ...
   <script type="text/javascript">
  			$(function(){
  			$('#dg').datagrid({    
			    url:"<%=request.getContextPath() %>/getDeptList",			       
			    columns:[[
			    	{field:"yyy",checkbox:true},    
			        {field:"dept_id",title:'员工ID',width:50},    
			        {field:"dept_name",title:'员工姓名',width:80},
			        {field:"xxx",title:'操作',width:100,formatter: function(value,row,index){
					return "<a href='javascript:void(0)'>修改</a>|<a href='javascript:void(0)' onclick='del("+row.cust_id+")'>删除</a>";}    
			    	}
			    ]],
			    // 隔行换色
		         striped:true,
 ...
 */		                
			...
			function save(){
				$("#myform").form("submit",{
				url:"<%=request.getContextPath() %>/addDept",
